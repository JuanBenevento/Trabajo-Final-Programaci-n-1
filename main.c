...
void agregarComentarioLista(stComentario comentarios[], int *cantidadComentario, stComentario nuevoComentario);
...
int iniciarSesion(const char *filename, stUsuario *usuarioLogueado) {
    ...
    ingresarContraseniaCamuflada(password);
...
void modificarInformacionPersonal(const char *filename, stUsuario *usuarioLogueado) {
    ...
    do{
        ...
        printf("\n***************************************************\n");
        printf("\n1- Modificar mail.\n");
        printf("2- Modificar contrasenia.(por lo menos una minuscula y una mayuscula)\n");
        printf("3- Modificar nombre.\n");
...
void eliminarLibroDefinitivamente(stLibro libros[], int *cantidad, int *idLibro, const char *librosFilename) {
    ...
    // Encontrar el índice del libro a eliminar
    for (int i = 0; i < *cantidad; i++) {
        if (libros[i].idLibro == *idLibro) {
            indiceEliminar = i;
            break;
    ...
    // Si se encontró el libro, proceder con la eliminación
    if (indiceEliminar != -1) {
        // Desplazar todos los elementos después del índice hacia atrás
        for (int j = indiceEliminar; j < *cantidad - 1; j++) {
            libros[j] = libros[j + 1];
        }
        (*cantidad)--;

        // Guardar los libros actualizados en el archivo
        guardarLibros(librosFilename, libros, *cantidad);
